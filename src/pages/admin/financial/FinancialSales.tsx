...
export default function FinancialSales() {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      let q = supabase.from("orders").select("*, order_items(*)").gte("created_at", from).lte("created_at", to).order("created_at", { ascending: false });
      const { data } = await q;
      setOrders(data || []);
    };
    fetch();
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Select value={paymentFilter} onValueChange={setPaymentFilter}>
          <SelectTrigger className="w-[160px] bg-card"><SelectValue placeholder="Pagamento" /></SelectTrigger>
          <SelectContent>
              ...
              <tbody>
                {filtered.map(o => (
                  <tr key={o.id} className="border-b hover:bg-muted/10 transition-colors">
                    ...
                    <td className="p-3">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${o.payment_status === "paid" ? "bg-emerald-100 text-emerald-700" : o.payment_status === "refunded" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}>
                        {o.payment_status}
                      </span>
                    </td>
            ...
            {filtered.map(o => (
              <div key={o.id} className="p-4 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${o.payment_status === "paid" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
                    {o.payment_status}
                  </span>
                </div>
