        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button size="sm"><Plus className="w-4 h-4 mr-1" />Registrar</Button>
            </DialogTrigger>
            <DialogContent className="max-w-md">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input value={form.order_id} onChange={e => setForm({ ...form, order_id: e.target.value })} placeholder="cole o ID do pedido" />
                </div>
                <div>
                  <Label>Valor</Label>
                ...
                <div className="flex items-center gap-2">
                  <input type="checkbox" checked={form.is_chargeback} onChange={e => setForm({ ...form, is_chargeback: e.target.checked })} />
                  <Label>É chargeback</Label>
                </div>
                <div>
                  <Label>Motivo</Label>
          ...
          <Button variant="outline" size="sm" onClick={() => exportToCsv("reembolsos", refunds.map(r => ({
                ...
                {refunds.map(r => (
                  <tr key={r.id} className="border-b hover:bg-muted/10">
                    ...
                    <td className="p-3 text-center">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${r.is_chargeback ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}>
                        {r.is_chargeback ? "Chargeback" : r.refund_type}
                      </span>
                    </td>
                    <td className="p-3 text-center">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${r.status === "paid" ? "bg-emerald-100 text-emerald-700" : r.status === "approved" ? "bg-blue-100 text-blue-700" : "bg-amber-100 text-amber-700"}`}>
                        {r.status}
                      </span>
                    </td>
