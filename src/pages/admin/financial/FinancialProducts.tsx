    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button variant="outline" size="sm" onClick={() => exportToCsv("produtos-financeiro", products.map(p => ({
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-1">
              {(["revenue", "unitsSold", "grossProfit"] as const).map(m => (
                <Button key={m} variant={metric === m ? "default" : "outline"} size="sm" onClick={() => setMetric(m)} className="text-xs">
                  {metricLabels[m]}
                </Button>
              ))}
            </div>
                ...
                {products.map(p => (
                  <tr key={p.id} className="border-b hover:bg-muted/10">
                    ...
                    <td className="p-3 text-right">
                      <span className={p.margin < 20 ? "text-destructive font-bold" : "text-emerald-600"}>
