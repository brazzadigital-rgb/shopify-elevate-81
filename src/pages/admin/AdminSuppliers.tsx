    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground font-sans">Gerencie seus fornecedores e parceiros</p>
        </div>
      ...
      <div className="relative max-w-sm">
        ...
        <Input placeholder="Buscar por nome ou CNPJ..." value={search} onChange={e => setSearch(e.target.value)} className="pl-10 rounded-xl" />
      </div>
        ...
        <table className="w-full text-sm font-sans">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              ...
              <th className="text-left px-4 py-3 font-semibold text-muted-foreground hidden md:table-cell">Prazo</th>
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr key={s.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                ...
                <td className="px-4 py-3 text-muted-foreground hidden md:table-cell">{s.shipping_days ? `${s.shipping_days}d` : "—"}</td>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><Label>Contato Responsável</Label><Input value={form.contact_person} onChange={e => setForm(f => ({ ...f, contact_person: e.target.value }))} /></div>
              <div><Label>Prazo Envio (dias)</Label><Input type="number" value={form.shipping_days} onChange={e => setForm(f => ({ ...f, shipping_days: Number(e.target.value) }))} /></div>
            </div>
            <div>
              <Label>Status</Label>
            ...
            <Button onClick={() => saveMutation.mutate()} disabled={!form.trade_name || saveMutation.isPending} className="bg-accent text-accent-foreground font-bold rounded-xl">
