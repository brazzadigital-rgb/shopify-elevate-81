...
export default function AdminRoles() {
  ...
  const { data: assignedUsers = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return rows.map((r: any) => ({
        ...r,
        full_name: profiles?.find((p: any) => p.user_id === r.user_id)?.full_name || "Sem nome",
      }));
    },
  });

  // Search users by name to assign
  ...
  const assignUserMutation = useMutation({
    ...
    onError: (err: any) => {
      toast({ title: "Erro", description: err.message?.includes("duplicate") ? "Usuário já possui esta função" : err.message, variant: "destructive" });
    },
  });
  ...
  const removeUserMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["admin-role-users"] });
      toast({ title: "Usuário removido da função" });
    },
  });
          ...
          {!selectedRole ? (
            ...
            <Tabs defaultValue="permissions">
              <TabsList className="mb-4">
                ...
                <TabsTrigger value="users">Usuários ({assignedUsers.length})</TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="users">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Button onClick={() => setAssignDialogOpen(true)} className="bg-accent text-accent-foreground font-bold rounded-xl gap-2" size="sm">
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            <div>
              ...
              <Input placeholder="Digite o nome do usuário..." value={searchEmail} onChange={e => setSearchEmail(e.target.value)} />
            </div>
                    ...
                    return (
                      <div key={u.user_id} className="flex items-center justify-between px-3 py-2 rounded-lg hover:bg-muted/50">
                        ...
                        {isAssigned ? (
                          ...
                          <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => assignUserMutation.mutate(u.user_id)}>
                            Atribuir
                          </Button>
                        )}
                      </div>
