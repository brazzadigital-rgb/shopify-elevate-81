...
export function HeaderBoutiqueClean() {
  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-50 bg-background border-b border-border/50 transition-all duration-300">
          ...
          <form onSubmit={handleSearch} className="hidden md:flex flex-1 max-w-xl mx-auto">
            <div className="relative w-full">
              <input
                ...
                className="w-full h-11 rounded-full border border-border bg-muted/30 text-foreground placeholder:text-muted-foreground text-sm font-sans pl-5 pr-12 focus:outline-none focus:ring-2 focus:ring-accent/20"
              />
          ...
          <div className="hidden md:flex items-center gap-4 shrink-0">
            ...
            {cartEnabled && (
              <button onClick={() => setIsOpen(true)} className="relative text-foreground/70 hover:text-accent transition-colors min-h-[unset] min-w-[unset]">
                ...
                {itemCount > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 rounded-full bg-accent text-accent-foreground text-[9px] font-bold w-[16px] h-[16px] flex items-center justify-center font-sans min-h-[unset]">
                    {itemCount}
                  </span>
                )}
              </button>
          ...
          <div className="flex md:hidden items-center gap-1">
            {cartEnabled && (
              <button onClick={() => setIsOpen(true)} className="relative w-10 h-10 flex items-center justify-center min-h-[unset] min-w-[unset]">
                ...
                {itemCount > 0 && (
                  <span className="absolute top-0 right-0 rounded-full bg-accent text-accent-foreground text-[9px] font-bold w-4 h-4 flex items-center justify-center font-sans min-h-[unset]">
                    {itemCount}
                  </span>
                )}
              </button>
        ...
        <form onSubmit={handleSearch} className="md:hidden px-4 pb-3">
          <div className="relative w-full">
            <input
              ...
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="O que está buscando?"
              className="w-full h-10 rounded-full border border-border bg-muted/30 text-foreground text-sm pl-4 pr-10 focus:outline-none focus:ring-2 focus:ring-accent/20"
            />
          ...
          <div className="container flex items-center justify-center gap-8 h-10">
            {navLinks.map((link) => (
              <Link
                key={link.to}
                to={link.to}
                className={`font-sans text-[12px] font-semibold uppercase tracking-[0.12em] transition-colors ${
                  location.pathname === link.to ? "text-accent" : "text-foreground/60 hover:text-foreground"
                }`}
              >
                {link.label}
              </Link>
            ...
            <>
              ...
              >
                ...
                <div className="p-4 space-y-2">
                  {accountEnabled && (
                    <Link to={user ? "/conta" : "/auth"} onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 p-3 rounded-xl bg-muted/50">
                      <User className="w-5 h-5 text-accent" /><span className="text-sm font-sans font-semibold">Minha conta</span>
                    </Link>
                  )}
                  {trackEnabled && (
                    <Link to="/rastreamento" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-3 p-3 rounded-xl bg-muted/50">
                      <MapPin className="w-5 h-5 text-accent" /><span className="text-sm font-sans font-semibold">Rastrear pedido</span>
                    </Link>
                  )}
                </div>
              </motion.div>
